                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center gap-2">
                                                       <a href="{{ url('admin/booking_detail', $booking->id)}}">
                                                            <div class="rounded bg-light avatar-md d-flex align-items-center justify-content-center">
                                                                 <img src="{{ $booking->getSelfieImages() }}" alt="{{ $booking->getSelfieImages() }}" class="avatar-md">
                                                            </div>
                                                       </a>
                                            ...
                                            <td>
                                                {{ $booking->f_name . ' '. $booking->l_name }}
                                            </td>
                                            <td>{{ $booking->phone }}</td>
                                            <td>{{ $booking->prop_location }}</td>
                                            <td>{{ $booking->prop_name }}</td>
                                            <td><span class="badge bg-{{ $booking->available == 1 ? 'success' : 'danger' }} text-light px-2 py-1 fs-13">{{ $booking->available == 1 ? 'Yes' : 'No' }}</span></td>
                                            <td>{{ Carbon::parse($booking->scheduled_date)->toDayDateTimeString() }}</td>
                                            <td>{{ $booking->viewd == 1 ? 'Yes' : 'No' }}</td>
                                            <td>{{ Carbon::parse($booking->created_at)->toDayDateTimeString() }}</td>
                                            <td>
                                                <div class="d-flex gap-2">
                                                  
                                                    <a href="{{ url('admin/property_detail', $booking->prop_id) }}" class="btn btn-light btn-sm"><iconify-icon icon="solar:eye-broken" class="align-middle fs-18"></iconify-icon></a>
                                                  
                                                    <a href="javascript:void(0);" class="btn btn-soft-danger btn-sm delete_booking_btn" data-data_id="{{ $booking->id }}" title="Delete Application"><iconify-icon icon="solar:trash-bin-minimalistic-2-broken" class="align-middle fs-18"></iconify-icon></a>
                                                </div>
                                            </td>
                                        </tr>
